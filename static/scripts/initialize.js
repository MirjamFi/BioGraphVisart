      ...
      if(val.length>0){
        ...
        // get min and max for legend node
          ...
          if(!isNaN(nodesMin)){
            ...
            cy.$('node['+nodeVal+' < "0"]').style('background-color', 'mapData('+nodeVal+','+ nodesMin+', 0, #006cf0, white)')
            .style('color', 'black')
            cy.$('node['+nodeVal+' <='+0.5*nodesMin+']').style('color', 'white')
            cy.$('node['+nodeVal+' > "0"]').style('background-color', 'mapData('+nodeVal+', 0,'+ nodesMax+', white, #d50000)')
            .style('color', 'black')
            cy.$('node['+nodeVal+' >='+0.5*nodesMax+']').style('color', 'white')
            cy.$('node['+nodeVal+' = "0"]').style('background-color', 'white').style('color','black')
        }
          if(isNaN(nodesMin)){
            cy.$('node[id = "l1"]')
